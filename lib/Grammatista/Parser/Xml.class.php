...
{
	const XMLNS_GRAMMATISTA_PARSER = 'urn:GrammatistaParserXml';
	...
	protected function load(GrammatistaEntity $entity)
	{
		...
		$this->xpath->registerNamespace('grammatista', self::XMLNS_GRAMMATISTA_PARSER);
	}
	...
	protected function tagElement(DOMElement $element)
	{
		...
		$tag = md5(uniqid());
		$element->setAttributeNS(self::XMLNS_GRAMMATISTA_PARSER, 'grammatista:tag', $tag);
		return $tag;
	}
	...
	protected function untagElement(DOMElement $element)
	{
		...
		$element->removeAttributeNS(self::XMLNS_GRAMMATISTA_PARSER, 'grammatista:tag');
	}
	...
	protected function findLine($tag, $elementName, array $namespaces = array())
	{
		...
		$xml.= sprintf('<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:grammatista="%1$s" xmlns:saxon="%2$s"', self::XMLNS_GRAMMATISTA_PARSER, self::XMLNS_SAXON);
